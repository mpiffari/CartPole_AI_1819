%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% LaTeX book template                           %%
%% Author:  Amber Jain (http://amberj.devio.us/) %%
%% License: ISC license                          %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[a4paper,11pt]{book}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{lmodern}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Source: http://en.wikibooks.org/wiki/LaTeX/Hyperlinks %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{hyperref}
\usepackage{graphicx}
\usepackage[english]{babel}
\usepackage[a4paper,top=2cm,bottom=2cm,left=2cm,right=2cm]{geometry}
\usepackage{lscape}
\usepackage{caption}
\usepackage{amsmath}
\usepackage{listingsutf8}

\usepackage{subfig}

\usepackage{wrapfig}
\usepackage{rotating}
\usepackage{epstopdf}

\usepackage[ruled]{algorithm2e}
\lstset{% general command to set parameter(s)
	basicstyle=\small, % print whole listing small
	numbers=left,
	keywordstyle=\color{black}\bfseries,
	% underlined bold black keywords
	identifierstyle=, % nothing happens
	stringstyle=\ttfamily} % typewriter type for strings

\lstset{language=Java} 
\captionsetup{tableposition=top,figureposition=bottom,font=small}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 'dedication' environment: To add a dedication paragraph at the start of book %
% Source: http://www.tug.org/pipermail/texhax/2010-June/015184.html            %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newenvironment{dedication}
{
   \cleardoublepage
   \thispagestyle{empty}
   \vspace*{\stretch{1}}
   \hfill\begin{minipage}[t]{0.66\textwidth}
   \raggedright
}
{
   \end{minipage}
   \vspace*{\stretch{3}}
   \clearpage
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Chapter quote at the start of chapter        %
% Source: http://tex.stackexchange.com/a/53380 %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\makeatletter
\renewcommand{\@chapapp}{}% Not necessary...
\newenvironment{chapquote}[2][2em]
  {\setlength{\@tempdima}{#1}%
   \def\chapquote@author{#2}%
   \parshape 1 \@tempdima \dimexpr\textwidth-2\@tempdima\relax%
   \itshape}
  {\par\normalfont\hfill--\ \chapquote@author\hspace*{\@tempdima}\par\bigskip}
\makeatother

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% First page of book which contains 'stuff' like: %
%  - Book title, subtitle                         %
%  - Book author name                             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Book's title and subtitle
\title{
	\includegraphics[width=0.7\textwidth]{Immagini/UniBg.png}
	\\ \Huge \textbf{Cart pole: implementazione di algoritmi per la gestione del pendolo inverso} \\ \huge \textit{\textbf{Documentazione progettuale}} \\ \bigskip \huge Progetto del corso di Intelligenza Artificiale \\ Università degli Studi di Bergamo \\ \huge A.A. 2018/2019}
% Author
\author{\textsc{Piffari Michele - 1040658}}

\begin{document}

	\frontmatter
	\maketitle
	\begin{flushright}
		\null\vspace{\stretch {1}}
		\emph{\textit{Reinforcement learning is learning what to do—how to map situations to actions—so as to maximize a numerical reward signal. The learner is not told which actions to take, but instead must discover which actions yield the most reward by trying them. (Sutton and Barto)}}
		\vspace{\stretch{2}}\null
	\end{flushright}
	
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% Auto-generated table of contents, list of figures and list of tables %
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\tableofcontents
	\listoffigures
	
	
	
	\mainmatter
	\chapter{Introduzione}
	\section{Cart Pole}
	\input{Capitoli/CartPole.tex}
	\section{RL e controllo}
	\input{Capitoli/RL_control.tex}
	\section{Cart Pole environments and reward}
	\input{Capitoli/CartPoleEnv.tex}
	
	
	\chapter{Implementazione}
	\section{Algoritmo Q learning}
	\input{Capitoli/QLearning.tex}
	\section{Algoritmo DQN}
	\input{Capitoli/DQNlearning.tex}
	\section{Algoritmo \textit{Finite Differences}}
	\input{Capitoli/FiniteDifferences.tex}
	
	\begin{thebibliography}{9}
		\bibitem{bib1}
		\emph{Reinforcement Learning, Part 1: What Is Reinforcement Learning?}
		\url{https://it.mathworks.com/videos/reinforcement-learning-part-1-what-is-reinforcement-learning-1551974943006.html}
		
		\bibitem{bb2}
		\emph{REINFORCEMENT LEARNING (DQN) TUTORIAL}
		\url{https://pytorch.org/tutorials/intermediate/reinforcement_q_learning.html}
		
		\bibitem{bb3}
		\emph{Deep Q Learning and Deep Q Networks (DQN) Intro and Agent - Reinforcement Learning w/ Python Tutorial p.5}
		\url{https://pythonprogramming.net/deep-q-learning-dqn-reinforcement-learning-python-tutorial/}
		
		\bibitem{bb4}
		\emph{An introduction to Q-Learning: reinforcement learning}
		\url{https://www.freecodecamp.org/news/an-introduction-to-q-learning-reinforcement-learning-14ac0b4493cc/}
		
		\bibitem{bb5}
		\emph{CartPole with Q-Learning - First experiences with OpenAI Gym}
		\url{https://muetsch.io/cartpole-with-qlearning-first-experiences-with-openai-gym.html}
		
		\bibitem{bb6}
		\emph{Reinforcement Learning in problemi di controllo del bilanciamento}
		\url{https://amslaurea.unibo.it/15539/1/tesiBuzzoniMichele.pdf}
		
		\bibitem{bb7}
		\emph{Simple Reinforcement Learning: Q-learning}
		\url{https://towardsdatascience.com/simple-reinforcement-learning-q-learning-fcddc4b6fe56}
		
		\bibitem{bb8}
		\emph{A Hands-On Introduction to Deep Q-Learning using OpenAI Gym in Python}
		\url{https://www.analyticsvidhya.com/blog/2019/04/introduction-deep-q-learning-python/}
		
				
		\bibitem{bb9}
		\emph{UCL Course on RL}
		\url{http://www0.cs.ucl.ac.uk/staff/d.silver/web/Teaching.html}
	\end{thebibliography}

\end{document}
